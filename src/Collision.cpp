...
/*	uses a certain threshold for the distance to the surface
...
bool vertexInTriangle(const Eigen::Vector3d &P, const Eigen::Vector3d &A, const Eigen::Vector3d &B, const Eigen::Vector3d &C, float &dist, Eigen::Vector3d &N){
	//(if point P collides with more then one triangle (only the first found triangle is considered))
	//better for each point of cloth do (and in inner loop for each triangle do)
	//const vector<Eigen::Vector3d> &mesh
	//for(){ //for each triangle in mesh do
		//Eigen::Vector3d A = Eigen::Vector3d(0.0,0.0,0.0);
		//Eigen::Vector3d B = Eigen::Vector3d(1.0,0.0,0.0);
		//Eigen::Vector3d C = Eigen::Vector3d(0.0,1.0,-1.0);
