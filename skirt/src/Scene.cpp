...
void Scene::update() {
	...
	for (size_t i = 0; i < model3dObjectsWithMass; ++i) {
		while (j < model3dObjects && model3dObjectOffset[j] < model3dMassRevOffsOrig[i]) {
...
//				<< ", model3dMassRevOffsOrig[i]" << model3dMassRevOffsOrig[i] << endl;//XXX
		...
		if (model3dMassRevOffsOrig[i] > 0) {
			cerr << "prev collision " << i << " - " << "[0 .. " << model3dObjectOffset[j]-1 << "]" << endl;//XXX
			std::collisionDetectionAndResponse(points,
		...
		if (model3dMassRevOffsOrig[i] < model3dObjectOffset[model3dObjects-1])
			cerr << "post collision " << i << " - " << "[" << model3dObjectOffset[next] << " .. " << model3dObjectOffset[next] + model3dVertices - model3dObjectOffset[next] -1 << "]" << endl;//XXX
			std::collisionDetectionAndResponse(points,
					model3dMassFwdOffs[i],
					model3dMasses[i],
					model3dPositions,
